...
module.exports.IPCValueToValue = function (v) {
  if (isRenderer()) return instantiateArrayBuffers(v)
...
function instantiateArrayBuffers (v) {
  if (!v) return v
  if (ArrayBuffer.isView(v)) {
    return v.buffer.slice(v.byteOffset, v.byteLength + v.byteOffset)
  }
  if (v && typeof v === 'object') {
    if (v.type === 'Buffer' && 'data' in v) {
      return new ArrayBuffer(v.data)
    }
    // recurse
    for (let k in v) {
