...
module.exports = function (channelName, manifest, methods, globalPermissionCheck) {
  ...
  ipcMain.on(channelName, async function (event, methodName, requestId, ...args) {
    ...
    // helper to send
    const send = function (msgName, err, value) {
    ...
    if (streamTypes[type]) {
      return await handleStream(event, args, streamTypes[type])
    }

    api.emit('error', new Error(`Invalid method type "${type}" for "${methodName}"`), arguments)
  })

  async function handleStream (event, args, createStreamEvents) {
    // call duplex
    let stream
    try {
    ...
    var events = createStreamEvents(events, stream, requestId)
  ...
  function hookUpEventsAndUnregister(stream, events) {
    Object.keys.forEach(key =>  stream.on(key, events[key]))

    stream.unregisterEvents = () => {
      Object.keys.forEach(key =>  stream.removeListener(key, events[key]))
    }
  }

  function createReadableEvents (event, stream, requestId) {
    return {
      data: chunk => send('stream-data', valueToIPCValue(chunk)),
      readable: () => send('stream-readable'),
  ...
  function createDuplexEvents (event, stream, requestId) {
    return Object.assign(
      createWritableEvents (event, stream, requestId),
      createReadableEvents (event, stream, requestId))
  }

  function createWritableEvents (event, stream, requestId) {
    return {
      onDrain: () => send('stream-drain'),
      onClose: () => send('stream-close'),
      onError: err => {
        stream.unregisterEvents()
        send('stream-error', (err) ? err.message : '')
      },
