...
module.exports = function (channelName, manifest, methods, globalPermissionCheck) {
  ...
  ipcMain.on(channelName, async function (event, methodName, requestId, ...args) {
    ...
    if (type == 'readable') {
      ...
      // hook up events
      let onData     = chunk => send('stream-data', valueToIPCValue(chunk))
      let onReadable = () => send('stream-readable')
      let onClose    = () => send('stream-close')
      let onError    = err => {
