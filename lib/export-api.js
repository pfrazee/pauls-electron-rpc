...
module.exports = function (channelName, manifest, methods, globalPermissionCheck) {
  ...
  if (isNodeProcess()) {
    ...
    mockedSender.send = (channelName, msgType, requestId, err, value) => {
      // console.log('export-api#send', {channelName, msgType, requestId, err, value})
      process.send({channelName, msgType, requestId, err, value})
    }
    channel = {
      onMessage (cb) {
