...
module.exports = function (channelName, manifest, methods, globalPermissionCheck) {
  ...
  function wrap(obj) {
    if (obj) {
      var wrappedType = null
      if (obj instanceof Readable) {
        wrappedType = 'readable'
      } else if (obj instanceof Writable) {
        wrappedType = 'writable'
      } else if (obj instanceof Duplex) {
        wrappedType = 'duplex'
      }

      if (wrappedType) {
        var id = streams.length
        streams.push(obj)
        return {wrappedType, id}
      }
    }
  ...
  channel.onMessage(async function (event, methodName, requestId, ...args) {
    ...
    args = args.map(x => IPCValueToValue(x, null))
    ...
    for (var k in streamTypes) {
      if (methodName == 'stream-request-open-' + k) {
        event.returnValue = true
        var stream = streams[args[0]]
        streams[args[0]] = null
        return setupStream(event, requestId, streamTypes[k], send, stream)
      }
    }
    ...
    if (type == 'sync') {
      // call sync
      try {
        event.returnValue = { success: valueToIPCValue(method.apply(event, args), wrap) }
      } catch (e) {
    ...
    if (type == 'async') {
      ...
      const replyCb = (err, value) => {
        ...
        send('async-reply', err, valueToIPCValue(value), wrap)
      }
      args.push(replyCb)
    ...
    if (type == 'promise') {
      ...
      // handle response
      p.then(
        value => send('async-reply', null, valueToIPCValue(value, wrap)),
  ...
  function setupStream (event, requestId, createStreamEvents, send, stream) {
  ...
  async function handleStream (event, method, requestId, args, createStreamEvents, send) {
    ...
    setupStream(event, requestId, createStreamEvents, send, stream)
  ...
  function createReadableEvents (event, stream, requestId, send) {
    return {
      data: chunk => send('stream-data', valueToIPCValue(chunk, null), undefined, true),
