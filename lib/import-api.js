...
module.exports = function (channelName, manifest, opts) {
  ...
  function wrap(obj) {
    if (obj && obj instanceof Object) {
      var fn = null
      if (obj.wrappedType == 'readable') {
        fn = createReadable
      } else if (obj.wrappedType == 'writeable') {
        fn = createWritable
      } else if (obj.wrappedType == 'duplex') {
        fn = createDuplex
      }

      if (fn) {
        var streamId = streams.length
        channel.send('stream-request-open-' + obj.wrappedType, streamId, obj.id)
        let stream = fn(streamId)
        streams.push(stream)
  ...
  var createAPIMethod = {
    sync: methodName => {
      return (...args) => {
        args = args.map(x => valueToIPCValue(x, null))
        ...
        if (typeof success !== "undefined")
          return IPCValueToValue(success, wrap)
        if (error)
      ...
      return (...args) => {
        ...
        var p = new Promise((resolve, reject) => {
          asyncCbs.push((err, value) => {
            ...
            else     resolve(value) // IPCValueToValue(value, wrap))
          })
        })
        if (opts.timeout)
          asyncCbTimeouts[requestId] = setTimeout(onTimeout, opts.timeout, requestId)
  ...
  function createReadable (streamId) {
    // hook up the readable
    let r = new Readable({
      objectMode: true,
      read() {}
    })
    r.close = (...args) => channel.send('stream-request-close', streamId, ...args)
    return r
  }
  ...
  function createWritable (streamId) {
    let w = new Writable({
      ...
      write (chunk, enc, next) {
        channel.send('stream-request-write', streamId, chunk, enc)
        next()
      }
    })
    w.end   = (...args) => channel.send('stream-request-end', streamId, ...args)
    w.close = (...args) => channel.send('stream-request-close', streamId, ...args)
    return w
  }

  function createDuplex (streamId) {
    // hook up the readable
    let r = new Readable({
      objectMode: true,
      read() {}
    ...
    stream.end   = (...args) => channel.send('stream-request-end', streamId, ...args)
    stream.close = (...args) => channel.send('stream-request-close', streamId, ...args)
    return stream
  }
  ...
  channel.onMessage(function onIPCMessage (event, msgType, requestId, ...args) {
    ...
    if (msgType == 'async-reply')
      return onCbReply(requestId, args.map(x => IPCValueToValue(x, wrap)))
    ...
    if (msgType.startsWith('stream-')) {
      ...
      if (msgType == 'stream-data')
        return stream.push(IPCValueToValue(args[0], wrap))
  ...
  function createError (error) {
    ...
    if (!copy) copy = new Error(error.message)
