...
module.exports = function (channelName, manifest, opts) {
  ...
  function createReadable (requestId) {
    ...
    r.close = (...args) => ipcRenderer.send(channelName, 'stream-request-close', requestId, ...args)
    return r
  }
  ...
  function createWritable (requestId) {
    let w = new Writable({
      ...
      write (chunk, enc, next) {
        ipcRenderer.send(channelName, 'stream-request-write', requestId, chunk, enc)
        next()
      }
    })
    w.end   = (...args) => ipcRenderer.send(channelName, 'stream-request-end', requestId, ...args)
    w.close = (...args) => ipcRenderer.send(channelName, 'stream-request-close', requestId, ...args)
    return w
  }
  ...
  function createDuplex (requestId) {
    ...
    stream.end   = (...args) => ipcRenderer.send(channelName, 'stream-request-end', requestId, ...args)
    stream.close = (...args) => ipcRenderer.send(channelName, 'stream-request-close', requestId, ...args)
    return stream
  }
